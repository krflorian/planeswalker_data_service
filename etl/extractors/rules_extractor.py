...
from ..objects.chroma_document import ChromaCollection
...
class RulesExtractor(Extractor):
    ...
    chapter_names: Optional[List] = Field(
        None, description="Collection object from ChromaDB"
    )
    ...
    def get_data(self):
        ...
        if (
            len(self.loader.collection.get(where={"documentType": "rule"})["ids"]) == 0
            or self.config["ETL_MODE"] == "full"
        ):
            self.full_extract()
        elif (
            self.loader.collection.metadata["lastUpdate"]
            < datetime.strptime(updates["creationDay"], "%Y-%m-%d").timestamp()
        ):
            self.delta_extract()
        else:
            print(
                f"collection {self.loader.collection_name} is up-to-date with latest data from {updates['creationDay']}"
            )
            return
    ...
    def full_extract(self):
        ...
        logging.info(
            f"Successfully extracted {len(self.rules)} rules and {len(self.chapter_names)} chapters"
        )
        ...
        if (
            ...
            for entry in updates["changes"]:
                ...
                # Print the distinct ruleNumbers
                    ...
                    logging.info(
                        f"Error deleting documents to the collection: {updates['changes']}."
                    )
                    continue
            ...
            for entry in updates["moves"]:
                try:
                    ...
                    logging.info(
                        f"Error deleting documents to the collection: {updates['moves']}."
                    )
                    continue
            ...
            logging.info(
                f"already up to date: current version: {updates['creationDay']}."
            )
            return
    ...
    def delta_extract(self, collection: Collection) -> list[ChromaCollection]:
        ...
        if ids:
            ...
            self.rules = {
                doc_id: document for doc_id, document in rules.items() if doc_id in ids
            }
        ...
        for rule in self.rules.values():
            document = ChromaCollection(
                ...
                metadata={
                    ...
                    "sectionTitle": str(
                        chapters.get(rule["ruleNumber"].split(".")[0])["sectionTitle"]
                    ),
                    "subsectionNumber": str(
                        chapters.get(rule["ruleNumber"].split(".")[0])[
                            "subsectionNumber"
                        ]
                    ),
                    "subsectionTitle": str(
                    ...
                    "combined_title": str(
                        chapters.get(rule["ruleNumber"].split(".")[0])["combined_title"]
                    ),
