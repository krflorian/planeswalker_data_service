...
class GlossaryExtractor(Extractor):
    ...
    def get_data(self):
        ...
        if (
            len(self.loader.collection.get(where={"documentType": "glossary"})["ids"])
            == 0
    ...
    def full_extract(self) -> list[ChromaCollection]:
        ...
        unoff_glossary = get_request(
            "https://api.academyruins.com/cr/unofficial-glossary"
        )
        logging.info(
            f"Successfully extracted {len(unoff_glossary)} unofficial-glossary entries"
        )
